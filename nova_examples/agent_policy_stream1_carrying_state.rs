    ...
    G::Scalar: PrimeField + PrimeFieldBits,
{
    ...
    fn synthesize<CS: ConstraintSystem<G::Scalar>>(
        ...
        // Witness:
        //
        // budget_next = budget + amount
        //
        let budget_next = AllocatedNum::alloc(
            cs.namespace(|| "budget_next"),
            || {
                let b = budget.get_value().unwrap();
                Ok(b + self.amount)
            },
        )?;
        ...
        // Enforce:
        //
        // budget + amount = budget_next
        //
        cs.enforce(
            || "budget_transition",

            |lc| {
                lc + budget.get_variable()
                   + amount.get_variable()
            },

            |lc| lc + CS::one(),

            |lc| lc + budget_next.get_variable(),
        );
        ...
        // budget_next < 8192
        //
        // (all bits >= 13 must be zero)
        //
        let budget_bits =
            budget_next.to_bits_le(
                cs.namespace(|| "budget_bits")
            )?;

        for (i, bit) in budget_bits.iter().enumerate().skip(9) { // 9 Bits <->  <=511
            Boolean::enforce_equal(
                cs.namespace(|| format!("budget_high_bit_{}", i)),
                bit,
                &Boolean::constant(false),
            )?;
        }
